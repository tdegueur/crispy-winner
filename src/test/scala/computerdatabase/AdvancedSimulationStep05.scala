...
class AdvancedSimulationStep05 extends Simulation {
    ...
    val edit = tryMax(2) { // let's try at max 2 times
      exec(http("Form")
        ...
        .exec(http("Post")
          ...
          check(status.is(session => 313)))//200 + ThreadLocalRandom.current.nextInt(2)))) // we do a check on a condition that's been customized with a lambda. It will be evaluated every time a user executes the request
  ...
  setUp(
    ...
    .protocols(httpConf)
    .assertions(
      global.successfulRequests.count.is(0)
    )
